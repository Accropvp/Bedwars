...
public class EmeraldSpawnPointCommand implements CommandExecutor {

    private final PersistentStorageHandler storage;

    public EmeraldSpawnPointCommand(PersistentStorageHandler storage) {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, String[] args) {
        if (args.length != 3) {
            sender.sendMessage("Usage: /emeraldSpawnPoint <x> <y> <z>");
            return true;
        }
        try {
            // Parse the arguments
